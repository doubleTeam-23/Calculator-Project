...
export default function MatrixCalculator({ className }: MatrixCalculatorProps) {
  ...
  const renderMatrixInput = (
    ...
    // 使用内联样式生成列数，避免 Tailwind 在运行时丢失动态类名的问题
    return (
      <div
        ...
        {Array.from({ length: size.rows }).map((_, i) => (
          Array.from({ length: size.cols }).map((_, j) => (
            <input
              key={`${i}-${j}`}
              type="number"
              // 使用可选链和空值合并，保证 0 正确显示而不是被视为 falsy
              value={matrix[i]?.[j] ?? ''}
              ...
              {matrix.map((row, i) => (
                <tr key={`row-${i}`}>
                  {Array.isArray(row) ? row.map((val, j) => {
                    // 保护性处理：只有在数字且有限时才 toFixed，否则安全地显示为字符串
                    const display = (typeof val === 'number' && Number.isFinite(val)) ? val.toFixed(2) : String(val ?? '');
                    return (
                      <td
                        key={`result-${i}-${j}`}
                        className="border border-gray-600 px-6 py-3 bg-gray-900 text-white text-center min-w-[60px]"
                      >
                        {display}
                      </td>
                    );
                  }) : (
                    <td key={`result-${i}-0`} className="border border-gray-600 px-6 py-3 bg-gray-900 text-white text-center min-w-[60px]">{String(row ?? '')}</td>
                  )}
                </tr>
              ))}
            </tbody>
  ...
  return (
    <div className={cn('w-full max-w-2xl mx-auto bg-gray-800 rounded-2xl shadow-2xl overflow-hidden', className)}>
      ...
      <div className="p-6 space-y-6">
             ...
             <div>
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="flex items-end">
                  <span className="text-white font-medium text-lg">{matrixASize.rows} × {matrixASize.cols}</span>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-8">
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                 <label className="block text-sm text-gray-400">矩阵A ({matrixASize.rows} × {matrixASize.cols})</label>
            ...
            <div>
              <div className="flex justify-between items-center mb-3">
                 <label className="block text-sm text-gray-400">矩阵B ({matrixBSize.rows} × {matrixBSize.cols})</label>
                <div className="flex items-center space-x-2">
                  <select
        ...
        {result !== null && (
           <div className="mt-6">
            <label className="block text-sm text-gray-400 mb-3">计算结果{Array.isArray(result) ? ` (${result.length} × ${result[0]?.length || 0})` : ''}</label>
